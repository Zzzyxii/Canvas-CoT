...
"""
    ...
    # Eval ID override (to resume into an existing run folder)
    parser.add_argument(
        '--eval-id',
        type=str,
        default=None,
        help='Override eval_id (e.g., T20251228_Gdd03fd1a) to resume writing into an existing run folder.'
    )
    # Infer + Eval or Infer Only
    parser.add_argument('--mode', type=str, default='all', choices=['all', 'infer'])
    # API Kwargs, Apply to API VLMs and Judge API LLMs
    parser.add_argument('--api-nproc', type=int, default=1, help='Parallel API calling')
    ...
    # Force rerun from a given dataset index (best-effort, mainly for API-model cache files)
    parser.add_argument(
        '--rerun-from',
        type=int,
        default=None,
        help=(
            'Force re-inference for samples with dataset index >= N (0-based) by pruning cached prediction files in the '
            'target eval folder before running. Intended for resuming an existing --eval-id run and redoing a tail.'
        ),
    )
    parser.add_argument(
    ...
    print(f"DEBUG: args.api_nproc = {args.api_nproc}")
    return args
...
def prune_cached_predictions_for_rerun(work_dir: str, model_name: str, dataset_name: str, rerun_from: int) -> None:
    if rerun_from is None:
        return

    supp_path = osp.join(work_dir, f'{model_name}_{dataset_name}_supp.pkl')
    if not osp.exists(supp_path):
        return

    backup_path = f'{supp_path}.bak_{_timestamp_for_backup()}'
    shutil.copy2(supp_path, backup_path)

    try:
        cached = load(supp_path)
    except Exception as err:
        print(f'WARNING: failed to load cache file for pruning: {supp_path} ({type(err).__name__}: {err})')
        return

    if not isinstance(cached, dict):
        print(f'WARNING: cache file is not a dict, skip pruning: {supp_path}')
        return

    kept = {}
    dropped = 0
    for k, v in cached.items():
        ik = _coerce_int_key(k)
        if ik is None:
            kept[k] = v
            continue
        if ik < rerun_from:
    ...
    dump(kept, supp_path)
    print(
        f'Pruned cached API predictions for rerun: kept={len(kept)}, dropped={dropped}, '
        f'rerun_from={rerun_from}. Backup: {backup_path}'
    )
...
def main():
    ...
    for _, model_name in enumerate(args.model):
        ...
        eval_id = args.eval_id if args.eval_id else f"T{date}_G{commit_id}"
        ...
        for _, dataset_name in enumerate(args.data):
            ...
            try:
                ...
                # If requested, prune cached API predictions so indices >= rerun_from are recomputed.
                # This is best-effort and safe (creates a backup first).
                if args.rerun_from is not None and RANK == 0:
                    prune_cached_predictions_for_rerun(
                        work_dir=pred_root,
