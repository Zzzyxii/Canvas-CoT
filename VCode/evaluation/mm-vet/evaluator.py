...
def arg_parser(prompt=prompt):
    ...
    "--mmvet_path",
    type=str,
    default="/path/to/mm-vet.json",
    help="Path to the mm-vet.json metadata file (e.g. /Path/to/mm-vet.json)",
    )
    parser.add_argument(
        "--result_file",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Custom base URL for the API endpoint",
    )
...
def load_metadata(args):
    ...
    mmvet_metadata = args.mmvet_path
    ...
    while need_more_runs(args, grade_results, len_data):
        for j in range(args.num_run):
            ...
            for id, line in tqdm(data.items()):
                ...
                model_pred = results.get(id, None)
                if model_pred is None:
                    if id not in grade_results:
                        grade_results[id] = {"model": [], "content": [], "score": []}
                    while len(grade_results[id]["score"]) < args.num_run:
                        grade_results[id]["model"].append("missing")
                        grade_results[id]["content"].append("no prediction")
                        grade_results[id]["score"].append(0.0)
                        ...
                        while flag:
                            try:
                                ...
                                if try_time > 5:
                                    score = 0.0
                                    flag = False
                                    print(f"[❌ Eval failed] Sample ID: {id}, Original response content: {repr(content)}")
