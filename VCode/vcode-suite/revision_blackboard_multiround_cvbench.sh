...
# ==============================================================================
#      🚀 AUTOMATED SVG OPTIMIZATION PIPELINE (BLACKBOARD MULTI-ROUND) - CV-Bench
# ==============================================================================
#
# Pipeline:
#   1) Multi-round blackboard revision of SVGs against CV-Bench reference images
...
# --- Inputs ---
# SVGs to optimize
INITIAL_SVG_FOLDER=".../cv-bench/SVG/GPT-5/generated_svgs"
# Reference CV-Bench images
ORIGINAL_IMAGES_FOLDER="data/cv-bench"

# --- Outputs (all under one root) ---
RESULT_ROOT="results-cvbench-gpt5/cv-bench"
...
echo "🚀 Starting Blackboard SVG Optimization Pipeline (CV-Bench)"
...
if [[ -d "$ANALYSIS_DIR_LOOP" ]]; then
    ...
    # Fix for nested directories (e.g. 2D/file.svg)
    find "$ANALYSIS_DIR_LOOP" -mindepth 1 -type d | while read -r dir; do
        rel_path="${dir#$ANALYSIS_DIR_LOOP/}"
        if [[ -f "$WORKSPACE_DIR/$rel_path.svg" ]]; then
            echo "   -> Skipping $rel_path.svg (already in analysis outputs)"
            rm -f "$WORKSPACE_DIR/$rel_path.svg"
        fi
    done
fi
# --------------------
