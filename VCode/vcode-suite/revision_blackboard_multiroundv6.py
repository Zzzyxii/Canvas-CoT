...
def _try_import_blackboard_model():
    ...
    try:
        from CanvasModule.openai_model_black_v6_2 import OpenAIModel as BlackboardOpenAIModel  # type: ignore
        from CanvasModule.openai_model_black_v6_2 import parse_response as blackboard_parse_response  # type: ignore
...
def blackboard_revise_svg_once(
    *,
    ...
    prompt = (
        ...
        "- If you think the image rendered by initial svg is worthless or completely wrong, you should use 'clear' to clear the canvas and generate a new svg for the whole original image to replace it, rather than call too many tools to revise the wrong one.\n\n"
        "CURRENT SVG CODE:\n"
        f"{current_svg_code}\n\n"
    ...
    if revised_svg:
        # Fix common case-sensitivity issues caused by HTML parsers or model output
        revised_svg = revised_svg.replace("viewbox=", "viewBox=")
        revised_svg = revised_svg.replace("preserveaspectratio=", "preserveAspectRatio=")
